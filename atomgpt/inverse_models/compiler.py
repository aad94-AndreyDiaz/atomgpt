...
import tempfile
import sys
from atomgpt.inverse_models._utils2 import (
    Version,
    is_main_process,
    is_distributed,
...
import triton
import regex
from atomgpt.inverse_models.peft_utils import get_lora_layer_modules
    ...
    try:
        ...
        if new_module is None:
            try:
                ...
                raise RuntimeError(
                    f"Direct module loading failed for {name}: {e}"
                )
        pass
    finally:
...
def apply_fused_lm_head(forward):
    ...
    for cross_entropy_find, cross_entropy_replacement in ce_finders:
        ...
        cross_entropy_find = (
            ...
            .replace(
                "$LABELSDEVICE$", r"(?:labels = labels\.to\([^\)]{1,}\))?"
            )
            .replace(
                "$LOGITSCALINGMULTIPLY$",
        ...
        # Find matches
        if (
            "loss\_function" in cross_entropy_find
            and "loss_function" not in forward
        ):
            continue
        elif (
            "loss\_function" not in cross_entropy_find
            and "loss_function" in forward
        ):
            continue
        elif (
        ...
        replacement = "\n".join(
            (len(spaces) - 4) * " " + x for x in replacement
        )
        replacement = (
            "logits = EMPTY_LOGITS\n"
            + (len(spaces) - 4) * " "
...
def convert_attention_masks_to_bool(module, old_source):
    ...
    if "return" not in splits:
        return old_source
    vars = re.findall(
        r"return[\s]{1,}(?:([^\,]{1,})\,[\s]{0,}){0,}([^\s]{1,})", splits
    )
    if len(vars) != 1:
        return old_source
    vars = vars[0]
    ...
    for var in vars:
        for split in all_splits:
            if re.search(
                re.escape(var) + ".+?" + r"torch\.finfo\(.+?\)\.min", split
            ):
...
def patch_gradient_checkpointing(module, source):
    ...
    modulelist_items = re.findall(
        r"(self\.[^\s]{1,}) = .*?nn\.ModuleList\(", init
    )
...
def patch_gradient_accumulation(modeling_file, module):
    ...
    for call_class, inner_class in inner_classes:
        ...
        has_kwargs = (
            tuple(inspect.signature(inner_class.forward).parameters.values())[
                -1
            ].kind
            == inspect._VAR_KEYWORD
        )
    ...
    source = inspect.getsource(module).replace(
        inspect.getsource(forward), source
    )
    return source
...
def unsloth_compile_transformers(
    ...
    exec(
        "model_logger.addFilter(HideLoggingMessage('`use_cache=True`'))",
        globals(),
        locals(),
    )
    ...
    functions = list(
        np.array(functions)[
            np.argsort([full_source.find(x) for x in functions])
        ]
    )
    ordered_functions = functions.copy()

    # Get class LlamaAttention(nn.Module)
    torch_modules = re.findall(
        r"class ([^\s]{1,})\(.+?\.Module\)", full_source
    )
    # Also get class LlamaSdpaAttention(LlamaAttention)
    inherited_class = (
        "(?:"
        + "|".join(
            re.findall(r"class ([^\s]{1,})\(.+?\.Module\)", full_source)
        )
        + ")"
    )
    inherited_modules = re.findall(
        r"class ([^\s]{1,})\(" + inherited_class + "\)", full_source
    )
    # OrderedSet
    ...
    for function in functions:
        # Start of text
        defined = re.findall(
            r"\bdef[\s]{1,}" + re.escape(function),
            full_source,
            flags=re.DOTALL,
        )
        # Disable self.
        called = re.findall(
            r"[\s]{1,}" + re.escape(function) + "\(.+?\)",
            full_source,
            flags=re.DOTALL,
        )
    ...
    if compile_attention:
        for (
            module,
            forward_source,
        ) in scaled_dot_product_attention_modules.items():
            if sdpa_gqa_replace:
    ...
    if fuse_lm_head:
        ...
        for module in modules:
            ...
            if (
                OLD_CUDA_ARCH_VERSION
                or OLD_TORCH_VERSION
                or OLD_TRITON_VERSION
            ):
                continue

            module_class = eval(f"modeling_file.{module}")
            if hasattr(module_class, "forward") and issubclass(
                module_class, GenerationMixin
            ):
                try:
